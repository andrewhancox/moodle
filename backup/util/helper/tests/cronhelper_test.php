...
class backup_cron_helper_testcase extends advanced_testcase {
    ...
    public function test_is_course_modified() {
        ...
        $this->waitForSecond();
        // Unmodified courses should not backed up.
        $this->assertFalse(testable_backup_cron_automated_helper::testable_is_course_modified($course->id, time()));

        $timepriortobackup = time();
        $this->waitForSecond();
        $otherarray = ['format' => backup::FORMAT_MOODLE,
